    ...
    >
      ...
      >
        <div className={`border-b border-slate-100 flex justify-between items-center shrink-0 ${fullscreen ? 'p-2 px-4 bg-white' : 'p-3 md:p-4 bg-slate-50/50'}`}>
          <h2 className={`font-black text-slate-600 tracking-tight ${fullscreen ? 'text-sm' : 'text-lg md:text-xl'}`}>{title}</h2>
          <button
            onClick={onClose}
        ...
        <div className={`flex-1 overflow-y-auto custom-scrollbar ${fullscreen ? 'p-0' : 'modal-body p-2 md:p-3'}`}>{children}</div>
      </div>
    </div>
  );
};
