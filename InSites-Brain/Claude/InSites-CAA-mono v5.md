...
- version: InSites-CAA-mono-v5.md
## Introduction
...
- The HTML artifact must use the template defined in §4 below, with D3 for rendering and AI queries per `[CA-AIQ]`.
...
- **typeof guard**: Always check `typeof L !== 'undefined'` (Leaflet), `typeof Chart !== 'undefined'` (Chart.js), etc. before initializing CDN-dependent features.
...
Brackets = conditional: Map only if `asset.coordinates.lat` is non-null; Themes only if ≥2 themes total across all categories; Vulnerability only if data exists; KG only if generated during session. AI Query is always present.
