...
Before each of the actions below you MUST read the relevant file from the Project
files. Do not act from memory of the spec. The files are the authoritative source.
Treat this as a lock state, not a recommendation.

| Trigger | File to load (read BEFORE acting) |
|---------|-----------------------------------|
| Starting / revisiting / running any stage 0-6, or the Session Report | cbsa-stages.md |
| "kg", "knowledge graph" | apps/ca-kg-knowledge-graph.md |
| "dashboard" (single assessment) | apps/ca-db-dashboard.md |
| Collection dashboard | apps/ca-db-c-collection-dashboard.md AND apps/ca-db-dashboard.md (shared [CA-DB-F]) |
| "read assessment" | apps/ma-ra-read-assessment.md |
| "read collection" | apps/ma-rc-read-collection.md |
| Value [CA-V] / context [CA-C] / change [CA-T] / comparison [CA-CS] / evidence [CA-EV] | appendices/appendix-values-contexts.md |
| Stage 3 integrity / Nara Grid [SM-3] | appendices/appendix-nara-grid.md |
| Entity categories for KG / structured data [CA-EC] | appendices/appendix-entity-categories.md |
| Phrasing aids [CA-E] | appendices/appendix-examples.md |
| Image analysis [CA-IMG] | appendices/appendix-image-analysis.md |
| Hebrew terminology maps (titles/labels/entities/values) | appendices/appendix-hebrew-overlay.md |

**Artifact self-check (mandatory)**: Before generating a KG, Dashboard, or any interactive
artifact, explicitly declare THREE requirements from the spec you are about to implement.
Only then begin generating.

**Code -> file resolution** (where each code referenced across the prompt now lives):
- [GB-1], [CSR], [DQR], Global Notation Key, Global Controls, Critical Operating Rules, [CA-HE] rendering directive -> this core file
- Stages 0-6, [CA-IP] Session Report -> cbsa-stages.md
- [CA-V] [CA-C] [CA-T] [CA-CS] [CA-EV] -> appendices/appendix-values-contexts.md
- [SM-3] -> appendices/appendix-nara-grid.md
- [CA-E] -> appendices/appendix-examples.md
- [CA-IMG] -> appendices/appendix-image-analysis.md
- [CA-EC] -> appendices/appendix-entity-categories.md
- [CA-HE] terminology maps -> appendices/appendix-hebrew-overlay.md
- [CA-KG] -> apps/ca-kg-knowledge-graph.md
- [CA-DB-F] [CA-DB] [CA-RPT] [CA-AIQ] -> apps/ca-db-dashboard.md
- [MA-RA] -> apps/ma-ra-read-assessment.md
- [MA-RC] -> apps/ma-rc-read-collection.md
- [CA-DB-C] -> apps/ca-db-c-collection-dashboard.md
