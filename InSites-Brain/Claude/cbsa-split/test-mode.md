...
- **Language**: follow the user's trigger language — English `/test` → English output; Hebrew `בדיקה מלאה` → Hebrew output per [CA-HE] in cbsa-core.md. You may quote Hebrew phrases from the source where precision helps, regardless of output language.
- **After Stage 6**: automatically generate the **Knowledge Graph** ([CA-KG] in ca-kg.md) and the **Assessment Dashboard** ([CA-DB] in ca-db.md) from the run — no need to ask first (this is a test). Both are native React artifacts.
